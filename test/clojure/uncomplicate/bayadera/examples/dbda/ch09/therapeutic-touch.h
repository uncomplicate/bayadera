inline REAL touch_logpdf(__constant const REAL* params, REAL* x) {
    ...
    REAL logp = beta_log(a, b, omega) - lbeta(a, b)
        + gamma_log(params[2], params[3], kappam2);

    for (uint i = 0; i < (DIM - 2); i++) {
        const REAL theta = x[i];
        valid = valid && (0.0f <= theta) && (theta <= 1.0f);
        logp += (beta_log(ak, bk, theta) - lbeta(ak, bk));
    }
