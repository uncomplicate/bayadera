...
(defn analysis []
  (with-default-bayadera
    (with-release [prior (distribution rhlr-prior)
                   prior-dist (prior (sv (op [2 0.01 1000] (take 100 (interleave (repeat 0) (repeat 100) (repeat 3) (repeat 10))))))
                   post (posterior "rhlr" (rhlr-likelihood (dim params)) prior-dist)
                   post-dist (post params)
                   post-sampler (sampler post-dist {:limits (sge 2 52 (op [1 20 0.001 100] (take 100 (interleave (repeat -100) (repeat 100) (repeat -3) (repeat 9)))))})]
