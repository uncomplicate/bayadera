...
// ============= With params ========================================
    ...
    REAL binomial_mcmc_logpdf(const uint32_t data_len, const uint32_t params_len, const REAL* params,
                              const uint32_t dim, const REAL* k) {
        return binomial_log_unscaled(params[0], params[1], k[0]);
    }

    REAL binomial_logpdf(const uint32_t data_len, const uint32_t params_len, const REAL* params,
                         const uint32_t dim, const REAL* k) {
        return binomial_log(params[0], params[1], k[0]);
    }

    REAL binomial_loglik(const uint32_t data_len, const REAL* data, const uint32_t dim, const REAL* p) {
