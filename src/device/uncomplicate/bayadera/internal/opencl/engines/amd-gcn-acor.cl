...
__kernel void acor_1d (const uint stride, const uint dim_id, const uint lag,
    ...
    if (local_id == 0) {
        while ((pc0 = atomic_xchg(&c0acc[dim_id],atomic_xchg(&c0acc[dim_id], 0.0f)+pc0))!=0.0f);
        while ((xacc = atomic_xchg(&dacc[dim_id],atomic_xchg(&dacc[dim_id], 0.0f)+xacc))!=0.0f);
    }
...
/*     /\*     work_group_barrier(CLK_LOCAL_MEM_FENCE); *\/ */
/*     /\* } *\/ */
/*     lacc[local_id] = c0acc[local_id]; */
/*     lacc[local_size + local_id] = dacc[local_id]; */
/*     work_group_barrier(CLK_LOCAL_MEM_FENCE); */

/*     if (gid == 0) { */
/*         for (uint i = 0; i < acor_dim; i++) { */
/*             if (i < local_size) { */
/*                 c0 += lacc[i]; */
/*                 d += lacc[local_size + i]; */
/*             } else { */
...
__kernel void acor (const uint n, const uint lag, const uint min_lag, const uint win_mult,
    ...
    while (dim_id < dim) {
        ...
        while ((min_lag < lag2) && (lag2 < tau * win_mult)) {
            ...
            n2 /= 2;
            const uint wgs = (WGS < n2) ? WGS : n2;
            const uint grid_dim = (n2 - 1) / wgs + 1;
            ...
            enqueue_kernel(queue, CLK_ENQUEUE_FLAGS_NO_WAIT, ndrange_1D(n2, wgs),
                           1, &sum_pairwise_event, &acor_event,
                           ^(local void *acc)
                           {acor_1d(stride * dim, dim_id, lag2, c0acc, dacc, means, acc);},
                           2*WGS);
            enqueue_marker(queue, 1, &acor_event, &marker);
            release_event(sum_pairwise_event);
            release_event(acor_event);
            release_event(marker);
            tau = dacc[dim_id] / c0;
