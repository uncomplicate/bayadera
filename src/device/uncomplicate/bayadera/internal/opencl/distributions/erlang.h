...
REAL erlang_mcmc_logpdf(const uint params_len, const REAL* params, const uint dim, const REAL* x) {
    return erlang_log_unscaled(params[0], params[1], x[0]);
}


REAL erlang_logpdf(const uint params_len, const REAL* params, const uint dim, const REAL* x) {
